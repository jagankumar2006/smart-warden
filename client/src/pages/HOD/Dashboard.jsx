...
const HodDashboard = () => {
  ...
  const handleAction = async (id, action, reason = '') => {
    setLoadingId(id);
    try {
      const status = action === 'approve' ? 'PENDING_WARDEN' : 'REJECTED';
      const body = { status };
  ...
  const historyPasses = passes.filter(p => p.status !== 'PENDING_HOD');
  ...
  return (
    <div className="space-y-10">
      <div>
        <h2 className="text-2xl font-bold dark:text-white">Department Dashboard</h2>
        <p className="text-gray-500 dark:text-gray-400">Overview and analytics for {user?.department || 'your department'}</p>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {pendingPasses.length === 0 ? (
                ...
                pendingPasses.map((pass) => (
                  <motion.div key={pass.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6 flex flex-col">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 rounded-full bg-primary-100 dark:bg-primary-900/50 flex items-center justify-center text-primary-600 dark:text-primary-400 overflow-hidden">
                    ...
                    <div className="space-y-3 flex-1">
                      ...
                      {pass.document_url && (
                        <a href={(pass.document_url?.startsWith('http') ? pass.document_url : `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}${pass.document_url}`)} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm text-primary-500 hover:text-primary-600">
                          <FileText size={16} className="mr-1" /> View Attached Document
                        </a>
                      )}
                    </div>

                    <div className="mt-6 flex space-x-3 pt-4 border-t border-gray-100 dark:border-gray-800">
                      <button disabled={loadingId === pass.id} onClick={() => { const reason = prompt('Enter rejection reason (optional):'); if (reason !== null) handleAction(pass.id, 'reject', reason); }} className="flex-1 flex items-center justify-center py-2 bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/40 rounded-xl font-medium transition-colors">
                        <X size={18} className="mr-1" /> Reject
                      </button>
                      <button disabled={loadingId === pass.id} onClick={() => handleAction(pass.id, 'approve')} className="flex-1 flex items-center justify-center py-2 bg-green-500 text-white hover:bg-green-600 rounded-xl font-medium transition-colors shadow-lg shadow-green-500/20">
